    ...
    def sample_tokens(
        ...
        if self.num_speculative_steps > 0:
            ...
            if self.speculator is not None and hasattr(
                self.speculator, "take_last_scheduled_draft_lengths"
            ):
                draft_token_counts = self.speculator.take_last_scheduled_draft_lengths()
                if draft_token_counts is not None:
                    async_output.set_draft_token_counts(
                        input_batch.req_ids,
                        draft_token_counts,
                        self.main_stream,
