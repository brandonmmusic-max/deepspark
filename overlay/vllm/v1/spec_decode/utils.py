...
def copy_and_expand_dspark_inputs_kernel(
    ...
    out_query_positions_ptr,  # [num_query_total] (output)
    out_context_slot_mapping_ptr,  # [num_context] (output)
    out_context_commit_mask_ptr,  # [num_context] (output)
    out_query_slot_mapping_ptr,  # [num_query_total] (output)
    out_token_indices_ptr,  # [num_reqs * num_speculative_tokens] (output)
    # Block table
    block_table_ptr,  # [max_reqs, max_blocks]
    ...
    """
    Fused kernel for DSpark first-pass input setup.

    DSpark differs from DFlash by sampling the anchor query position itself.
    The query input is [next_token, noise, ...] with exactly
    num_speculative_tokens positions, and every query position contributes one
    speculative draft token.
    """
    req_idx = tl.program_id(axis=0)
    block_idx = tl.program_id(axis=1)
    ...
    tl.store(out_context_slot_mapping_ptr + ctx_pos_out, slot, mask=is_ctx)
    tl.store(
        out_context_commit_mask_ptr + ctx_pos_out,
        j < num_committed_ctx,
        mask=is_ctx,
    )
    tl.store(out_query_slot_mapping_ptr + query_out, slot, mask=is_query)
