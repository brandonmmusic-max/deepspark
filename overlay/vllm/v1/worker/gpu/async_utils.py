...
class AsyncOutput(AsyncModelRunnerOutput):
    def __init__(
        ...
        self.draft_token_count_req_ids: list[str] | None = None
        self.draft_token_counts: np.ndarray | None = None
        self.draft_token_counts_event: torch.cuda.Event | None = None
    ...
    def set_draft_token_counts(
        self,
        req_ids: list[str],
        draft_token_counts: torch.Tensor,
        main_stream: torch.cuda.Stream,
        copy_stream: torch.cuda.Stream,
    ) -> None:
        self.draft_token_count_req_ids = req_ids.copy()
        self.draft_token_counts_event = torch.cuda.Event()
        with stream(copy_stream, main_stream):
            copy_stream.wait_stream(main_stream)
            self.draft_token_counts = async_copy_to_np(draft_token_counts)
            draft_token_counts.record_stream(copy_stream)
            self.draft_token_counts_event.record(copy_stream)
    ...
    def get_output(self) -> ModelRunnerOutput:
        ...
        if (
            self.draft_token_counts is not None
            and self.draft_token_count_req_ids is not None
        ):
            assert self.draft_token_counts_event is not None
            self.draft_token_counts_event.synchronize()
            self.model_runner_output.draft_token_counts = {
                req_id: int(count)
                for req_id, count in zip(
                    self.draft_token_count_req_ids,
                    self.draft_token_counts.tolist(),
                )
            }
