...
def _get_spec_decode_query_len_buckets(decode_query_len: int) -> tuple[int, ...]:
    raw = os.getenv("VLLM_SPEC_DECODE_QUERY_LEN_BUCKETS", "")
    buckets = {decode_query_len}
    if raw.strip():
        for part in raw.replace(";", ",").replace(" ", ",").split(","):
            ...
            try:
                ...
                logger.warning_once(
                    "Ignoring invalid VLLM_SPEC_DECODE_QUERY_LEN_BUCKETS "
                    "entry %r in %r.",
                    part,
                    raw,
                )
                continue
            if 1 <= bucket <= decode_query_len:
                buckets.add(bucket)
            else:
                logger.warning_once(
                    "Ignoring out-of-range spec-decode query bucket %s; "
                    "expected 1..%s.",
                    bucket,
                    decode_query_len,
                )
...
def _get_spec_decode_bucket_req_counts(max_num_reqs: int) -> tuple[int, ...]:
    raw = os.getenv("VLLM_SPEC_DECODE_BUCKET_REQ_COUNTS", "1,2,4,8")
    ...
    for part in raw.replace(";", ",").replace(" ", ",").split(","):
        ...
        try:
            ...
            logger.warning_once(
                "Ignoring invalid VLLM_SPEC_DECODE_BUCKET_REQ_COUNTS "
                "entry %r in %r.",
                part,
                raw,
            )
            continue
        if 1 <= req_count <= max_num_reqs:
            req_counts.add(req_count)
        else:
            logger.warning_once(
                "Ignoring out-of-range spec-decode bucket request count %s; "
                "expected 1..%s.",
                req_count,
                max_num_reqs,
            )
    return tuple(sorted(req_counts or {1, 2, 4, 8}))
...
class CudaGraphManager:
    def __init__(
        ...
        is_target_spec_decode = (
            spec_config is not None
            and spec_config.num_speculative_tokens is not None
            and decode_query_len == spec_config.num_speculative_tokens + 1
        )
        if is_target_spec_decode:
            self.uniform_decode_query_lens = _get_spec_decode_query_len_buckets(
                decode_query_len
            )
        else:
            self.uniform_decode_query_lens = (decode_query_len,)
        self.bucket_req_counts = _get_spec_decode_bucket_req_counts(
            self.max_num_reqs
        )
    ...
    def _init_candidates(self) -> None:
        ...
        extra_decode_sizes_by_query_len: dict[int, set[int]] = {}
        if len(self.uniform_decode_query_lens) > 1:
            max_capture_size = capture_sizes[-1]
            decode_sizes = set()
            for query_len in self.uniform_decode_query_lens:
                if query_len == self.decode_query_len:
                    continue
                sizes = {
                    query_len * num_reqs
                    for num_reqs in self.bucket_req_counts
                    if query_len * num_reqs <= max_capture_size
                }
                extra_decode_sizes_by_query_len[query_len] = sizes
                decode_sizes |= sizes
            capture_sizes = sorted(set(capture_sizes) | decode_sizes)
            logger.info(
                "Spec-decode CUDA graph query-length buckets enabled: %s "
                "for request counts %s",
                list(self.uniform_decode_query_lens),
                list(self.bucket_req_counts),
            )
            ...
            if separate_decode_routine and decode_mode:
                for query_len in self.uniform_decode_query_lens:
                    if (
                        ...
                        and num_tokens
                        not in extra_decode_sizes_by_query_len.get(query_len, set())
                    ):
                        continue
                    if (
                        query_len <= num_tokens <= max_decode_tokens
