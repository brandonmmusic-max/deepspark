...
class SpeculativeConfig:
    ...
    dspark_confidence_schedule: bool = False
    """Enable DSpark confidence-scheduled verification. When disabled, DSpark
    keeps the P1 fixed-length speculative verification behavior."""

    dspark_sts_temperatures: list[float] | None = None
    """Optional per-position Sequential Temperature Scaling temperatures for
    DSpark confidence logits. If absent, the DSpark proposer uses a conservative
    built-in default and logs that the default is active."""

    dspark_scheduler_knee_tokens: float = Field(default=64.0, gt=0.0)
    """Default normalized SPS curve knee used by the DSpark hardware-aware
    prefix scheduler when no profiled table is supplied."""

    dspark_scheduler_cost_exponent: float = Field(default=2.0, gt=0.0)
    """Default normalized SPS curve exponent used by the DSpark hardware-aware
    prefix scheduler when no profiled table is supplied."""
    ...
    def compute_hash(self) -> str:
        ...
        if uses_aux_hidden_states and self.draft_model_config is not None:
            ...
            if not layer_ids:
                ...
                if not layer_ids:
                    dspark_layer_ids = getattr(
                        self.draft_model_config.hf_config,
                        "dspark_target_layer_ids",
                        None,
                    )
                    if dspark_layer_ids:
                        # DSpark config follows the reference implementation's
                        # zero-based layer ids; vLLM aux outputs are captured
                        # after layer N, so add one.
    ...
    def hf_config_override(hf_config: PretrainedConfig) -> PretrainedConfig:
        ...
        if hf_config.model_type == "deepseek_v4":
            if getattr(hf_config, "dspark_block_size", 0):
                ...
                dspark_num_layers = getattr(hf_config, "dspark_num_layers", None)
                if dspark_num_layers is None:
                    dspark_num_layers = getattr(hf_config, "n_mtp_layers", None)
                if dspark_num_layers is None:
                    dspark_num_layers = 3
                hf_config.update(
                    {
                        "n_predict": n_predict,
                        "dspark_num_layers": int(dspark_num_layers),
                        "num_nextn_predict_layers": int(dspark_num_layers),
    ...
    def _verify_args(self) -> Self:
        ...
        if self.dspark_sts_temperatures is not None:
            if len(self.dspark_sts_temperatures) != self.num_speculative_tokens:
                raise ValueError(
                    "dspark_sts_temperatures must have length "
                    f"{self.num_speculative_tokens}, got "
                    f"{len(self.dspark_sts_temperatures)}."
                )
            if any(t <= 0.0 for t in self.dspark_sts_temperatures):
                raise ValueError("dspark_sts_temperatures entries must be > 0.")

        if self.dspark_confidence_schedule and self.method != "dspark":
            raise ValueError(
                "dspark_confidence_schedule is only valid with method='dspark'."
            )
